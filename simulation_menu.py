...
def run_custom_simulation():
    ...
    for r in range(1, rounds + 1):
        ...
        print(f"\n--- ROUND {r:2d} --- P(cartel)={g['posterior_collusion']:.3f} "
              f"alarm={'YES' if g['alarm'] else 'no'} | Buyer Cost=${g['winning_bid']:.2f} | AllocEff={alloc_eff}")
        ...
        for idx, (agent, info) in enumerate(sorted_agents[:5]):
            ...
            print(f"    {idx+1:02d}. {agent} {role:8} | markup={info['markup']:.3f}x | "
    ...
    fig.suptitle(f'Custom Simulation Results ({cartel_count} Cartel vs {honest_count} Honest)', fontsize=14)
    
    # 1. Left Plot: Buyer Cost & Alarm over time
    ax1 = axes[0]
    ax1.plot(df['Round'], df['BuyerCost'], marker='o', color='tab:green', label='Buyer Cost')
    ...
    ax1_twin = ax1.twinx()
    ax1_twin.bar(df['Round'], df['Alarm'], color='red', alpha=0.3, label='Alarm Triggered')
    ...
    ax2.pie(winner_counts, labels=winner_counts.index, autopct='%1.1f%%', colors=['coral', 'skyblue'])
    ...
    ax3.scatter(honest_bids['Round'], honest_bids['Bid'], color='skyblue', alpha=0.6, label='Honest Bids')
    ax3.scatter(cartel_bids['Round'], cartel_bids['Bid'], color='coral', alpha=0.6, label='Cartel Cover Bids', marker='x')
    ...
    plt.tight_layout()
    plot_path = "custom_simulation_plot.png"
